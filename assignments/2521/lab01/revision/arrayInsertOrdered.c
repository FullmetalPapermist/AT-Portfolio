...
void insertOrdered(int *arr, int size, int value) {
	...
	int *arrCopy = malloc(sizeof(int) * (size + 1));
	
	// copy array with slotted value
	int i = 0; 
	int isInserted = 0;
	
	for (int iCopy = 0; iCopy < size + 1; iCopy++) {
		if (iCopy == size && !(isInserted)) {
			arrCopy[iCopy] = value;
			isInserted = 1;

		} else if (value <= arr[i] && !(isInserted)) {
			arrCopy[iCopy] = value;
			isInserted = 1;
