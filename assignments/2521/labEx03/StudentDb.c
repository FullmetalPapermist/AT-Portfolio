...
int compareByName(Record r1, Record r2) {
    int nameComp = strcmp(RecordGetFamilyName(r1), RecordGetFamilyName(r2));
...
List DbFindByName(StudentDb db, char *familyName, char *givenName) {
    Record dummy = RecordNew(MIN_ZID, familyName, givenName);
    Record dummy2 = RecordNew(MAX_ZID, familyName, givenName);
    List names = TreeSearchBetween(db->byName, dummy, dummy2);
    RecordFree(dummy);
    RecordFree(dummy2);
    return names;
}
