...
def main():
    ...
    # your code goes here
    cur.execute("""
        SELECT p.code, s.code, p.id, s.id
        FROM Programs AS p
    ...
    if prog_info is None:
        prog_info = get_program(conn, row[0])
        prog_code = prog_info[1]

    if strm_info is None:
        strm_info = get_stream(conn, row[1])
        strm_code = strm_info[1]
    ...
    cur.execute("""
        ...
        WHERE rtype = 'stream'
        AND for_program = %s
        AND %s = ANY (string_to_array(acadobjs, ',')::int[])
    """, (prog_id, strm_id, ))
    if not cur.fetchone():
    ...
    for code, term, title, mark, grade, uoc in rows:
        requirement = ""
        print(  f"{code} {term} {title:<40s}{mark:>3}"
                f"{grade:>2s}  {uoc:2d}uoc  {requirement}")
    cur.close()
    conn.close()
