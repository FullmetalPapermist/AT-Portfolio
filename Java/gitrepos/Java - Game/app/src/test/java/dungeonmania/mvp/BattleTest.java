...
public class BattleTest {
    ...
    @Test
    @Tag("11-17")
    @DisplayName("Test sceptre can mind control mercenaries for a certain number of ticks")
    public void mindControl() {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_battleTest_mindControl", "c_battleTest_mindControl");

        assertEquals(1, TestUtils.getEntities(res, "mercenary").size());

        // pick up key sun stone and wood
        ...
        // walk into mind controlled merc, a battle does not occur
        res = dmc.tick(Direction.RIGHT);
        assertEquals(0, res.getBattles().size());

        // Walk into merc a battle does occur (mind controlling runs out)
    ...
    @Test
    @Tag("11-18")
    @DisplayName("Test battle buffs for midnight armour")
    public void midnightArmourBattle() throws InvalidActionException {
        String config = "c_battleTest_midnightArmourBattle";
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_battleTest_midnightArmourBattle", config);
        ...
        double mercenaryAttack = Double.parseDouble(TestUtils.getValueFromConfigFile("mercenary_attack", config));
        double armourDefence = Double.parseDouble(TestUtils.getValueFromConfigFile("midnight_armour_defence", config));
        assertEquals((mercenaryAttack - armourDefence) / 10, -firstRound.getDeltaCharacterHealth(), 0.001);
    }

    @Test
    @Tag("11-17")
    @DisplayName("Test battle buffs for mind controlled allies")
    public void mindControlledBuffs() throws InvalidActionException {
        String config = "c_battleTest_mindControlBuffs";
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_battleTest_mindControlledBuffs", config);
        String mercId0 = TestUtils.getEntities(res, "mercenary").get(0).getId();
        assertEquals(new Position(3, 6), TestUtils.getEntities(res, "mercenary").get(0).getPosition());

        res = dmc.tick(Direction.RIGHT);
        res = dmc.tick(Direction.RIGHT);
        res = dmc.tick(Direction.RIGHT);
        res = dmc.build("sceptre");
        res = dmc.interact(mercId0);
        res = dmc.tick(Direction.RIGHT);
        ...
        double mercenaryAttack = Double.parseDouble(TestUtils.getValueFromConfigFile("mercenary_attack", config));
        double armourDefence = Double.parseDouble(TestUtils.getValueFromConfigFile("ally_defence", config));
        assertEquals((mercenaryAttack - armourDefence) / 10, -firstRound.getDeltaCharacterHealth(), 0.001);
    }
