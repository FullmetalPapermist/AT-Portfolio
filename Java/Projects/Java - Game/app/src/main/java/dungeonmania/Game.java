...
public class Game {
    ...
    public int tick() {
        ...
        sub = nextTickSub;
        tickCount++;
        getMapEntities(Switch.class).stream().forEach(s -> s.tick(map));
        getMapEntities(Wire.class).stream().forEach(s -> s.tick(map));
        getMapEntities(Bomb.class).stream().forEach(e -> e.checkIfActive(map));
        getMapEntities(SwitchDoor.class).stream().forEach(e -> e.checkIfActive(map));
        getMapEntities(LightBulb.class).stream().forEach(e -> e.checkIfActive(map));
        return tickCount;
    }
