...
import dungeonmania.entities.MovableEntity;
...
public class GameMap {
    ...
    public void moveTo(Entity entity, Position position) {
        ...
        removeNode(entity);
        ((PositionEntity) entity).setPosition(position);
        addEntity(entity);
        triggerOverlapEvent(entity);
    }

    public void moveTo(Entity entity, Direction direction) {
        if (!canMoveTo(entity, Position.translateBy(((PositionEntity) entity).getPosition(), direction)))
            return;
        triggerMovingAwayEvent(entity);
        removeNode(entity);
        ((PositionEntity) entity).setPosition(Position.translateBy(((PositionEntity) entity).getPosition(), direction));
        addEntity(entity);
        triggerOverlapEvent(entity);
    }
    ...
    private void triggerMovingAwayEvent(Entity entity) {
        ...
        getEntities(((PositionEntity) entity).getPosition()).forEach(e -> {
            if (e != entity && e instanceof MovableEntity)
                callbacks.add(() -> ((MovableEntity) e).onMovedAway(this, entity));
        });
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        getEntities(((PositionEntity) entity).getPosition()).forEach(e -> {
            if (e != entity && e instanceof OverlappableEntity)
                overlapCallbacks.add(() -> ((OverlappableEntity) e).onOverlap(this, entity));
        });
    ...
    public void removeNode(Entity entity) {
        Position p = ((PositionEntity) entity).getPosition();
    ...
    public void destroyEntity(Entity entity) {
        ...
        if (entity instanceof DestroyableEntity) {
            ((DestroyableEntity) entity).onDestroy(this);
        }
    }
    ...
    public List<Conductor> getConductors(Position p) {
        List<Entity> entities = getEntities(p).stream().filter(e -> (e instanceof Conductor))
                .collect(Collectors.toList());
        return entities.stream().map(e -> (Conductor) e).collect(Collectors.toList());
    }

    public List<Wire> getWires(Position p) {
        List<Entity> entities = getEntities(p).stream().filter(e -> (e instanceof Wire)).collect(Collectors.toList());
        return entities.stream().map(e -> (Wire) e).collect(Collectors.toList());
    }

    public <T extends Entity> List<String> getIds(Class<T> type) {
        return getEntities().stream().filter(type::isInstance).map(type::cast).map(e -> e.getId())
                .collect(Collectors.toList());
    }
