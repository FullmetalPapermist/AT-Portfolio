...
import dungeonmania.rules.And;
import dungeonmania.rules.CoAnd;
import dungeonmania.rules.LogicalRule;
import dungeonmania.rules.Or;
import dungeonmania.rules.Xor;
...
public class EntityFactory {
    ...
    public Sceptre buildSceptre() {
        int mindControlledDuration = config.optInt("mind_control_duration");
        return new Sceptre(mindControlledDuration);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "bomb":
            ...
            if (!logic.equals("")) {
                return new Bomb(pos, bombRadius, logicalRule);
            }
        ...
        case "light_bulb_off":
            return new LightBulb(pos, logicalRule);
        case "switch_door":
            return new SwitchDoor(pos, logicalRule);
        case "wire":
            return new Wire(pos);
        case "sun_stone":
