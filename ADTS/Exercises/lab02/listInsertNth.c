...
struct node *doListInsertNth(struct node *list, int n, int value, int pos) {
	if (list == NULL) {
		...
		list->next = doListInsertNth(list->next, n, value, pos + 1);
		return list;
	}
}

void listInsertNth(struct list *list, int n, int value) {
	list->head = doListInsertNth(list->head, n, value, 1);
}
